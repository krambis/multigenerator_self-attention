        ...
        try:
            while not coord.should_stop():
                ...
                for d_iter in range(self.disc_updates):
                    _d_opt, d_fk_loss_, d_rl_loss = self.sess.run([d_opt,self.d_fk_losses,self.d_rl_losses[0]])
                    d_fk_loss = [d_fk_loss_i[0] for d_fk_loss_i in d_fk_loss_] # extract only the first element
                    if self.d_clip_weights:
                        self.sess.run(self.d_clip)

                _g_opt, g_adv_loss, g_l1_loss_ = self.sess.run([g_opt, self.g_adv_losses[0],self.g_l1_losses])
                ...
                s = '{}/{} (epoch {}), d_rl_loss = {:.5f}, '.format(counter, config.epoch * num_batches, curr_epoch,  d_rl_loss)
                for nr in range(self.depth):
